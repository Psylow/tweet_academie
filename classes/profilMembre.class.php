...
class Membre
{
    ...
    {
        $checkFollowingOrNot = $this->bdd->prepare('select * from follow where id_user =? and follows like ?');
        $checkFollowingOrNot->execute(array($this->id, "'%" . $id . "%'"));
        if ($checkFollowingOrNot->rowCount() >= 1) {
            return false;
        } else {
            return true;
        }
